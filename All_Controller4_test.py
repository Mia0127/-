...
# Retrieve and parse AP data: inspect => Network => payload => find execUiQuery.xml that you need => put into the payload_data: 'whole string' + cookieStr
...
for i in range(len(df)):
    ...
    try :
        df['rx_time'][i] = int(re.findall("([0-9]+)\/", df['rx_time'][i])[0])/60000
    except Exception:
        df['rx_time'][i] = 0
    try :
        df['tx_time'][i] = int(re.findall("([0-9]+)\/", df['tx_time'][i])[0])/60000
    except Exception:
        df['tx_time'][i] = 0
    try :
        df['channel_interference'][i] = int(re.findall("([0-9]+)\/", df['channel_interference'][i])[0])/60000
    except Exception:
        df['channel_interference'][i] = 0
    try :
        df['channel_free'][i] = int(re.findall("([0-9]+)\/", df['channel_free'][i])[0])/60000
    except Exception:
        df['channel_free'][i] = 0
    try :
        df['channel_busy'][i] = int(re.findall("([0-9]+)\/", df['channel_busy'][i])[0])/60000
    except Exception:
        df['channel_busy'][i] = 0
    # Add total client number
    for i in range(len(df) - 1):
        if df['ap_name'][i] == df['ap_name'][i+1]:
            df['sta_count_all'][i] = int(df['sta_count'][i]) + int(df['sta_count'][i+1])
...
for i in range(len(data_json)):
    for j in range(len(data_json)):
        if(data_json[i]['ap_name']==data_json2[j]['ap_name']):                 
            data_json[i]['ap_model']=data_json2[j]['ap_model']
            data_json[i]['ap_deployment_mode']=data_json2[j]['ap_deployment_mode']
            data_json[i]['ap_group']=data_json2[j]['ap_group']
            data_json[i]['ap_serial_number']=data_json2[j]['ap_serial_number']
            data_json[i]['ap_ip_address']=data_json2[j]['ap_ip_address']
            data_json[i]['ap_status']=data_json2[j]['ap_status']
            data_json[i]['ap_state_reason']=data_json2[j]['ap_state_reason']
            data_json[i]['ap_provisioned']=data_json2[j]['ap_provisioned']
            data_json[i]['lms_ip']=data_json2[j]['lms_ip']
            data_json[i]['ap_active_aac']=data_json2[j]['ap_active_aac']
            data_json[i]['ap_cluster_name']=data_json2[j]['ap_cluster_name']
            data_json[i]['ap_cur_dual_5g_mode']=data_json2[j]['ap_cur_dual_5g_mode']
            data_json[i]['ap_tri_radio_mode']=data_json2[j]['ap_tri_radio_mode']
            data_json[i]['radio_count']=data_json2[j]['radio_count']
            data_json[i]['ap_uptime']=data_json2[j]['ap_uptime']
            data_json[i]['ap_datazone']=data_json2[j]['ap_datazone']
            data_json[i]['green_state']=data_json2[j]['green_state']
            break
# =============================================================================
...
for i in range(len(data_json3)):
    try:
        data_json3[i]['time_stamp'] = int(data_json3[i]['time_stamp'])
        data_json3[i]['client_ht_phy_type'] = int(data_json3[i]['client_ht_phy_type'])
        data_json3[i]['client_conn_port'] = int(data_json3[i]['client_conn_port'])
        data_json3[i]['client_conn_type'] = int(data_json3[i]['client_conn_type'])
        data_json3[i]['client_timestamp'] = int(data_json3[i]['client_timestamp'])
        data_json3[i]['client_health'] = int(data_json3[i]['client_health'])
        data_json3[i]['total_moves'] = int(data_json3[i]['total_moves'])
        data_json3[i]['successful_moves'] = int(data_json3[i]['successful_moves'])
        data_json3[i]['steer_capability'] = int(data_json3[i]['steer_capability'])
        data_json3[i]['channel'] = int(data_json3[i]['channel'])
        data_json3[i]['radio_band'] = int(data_json3[i]['radio_band'])
        data_json3[i]['speed'] = int(data_json3[i]['speed'])
        data_json3[i]['max_negotiated_rate'] = int(data_json3[i]['max_negotiated_rate'])
        data_json3[i]['noise_floor'] = int(data_json3[i]['noise_floor'])
        data_json3[i]['radio_ht_phy_type'] = int(data_json3[i]['radio_ht_phy_type'])
        data_json3[i]['snr'] = int(data_json3[i]['snr'])
        data_json3[i]['total_data_frames'] = int(data_json3[i]['total_data_frames'])
        data_json3[i]['total_data_bytes'] = int(data_json3[i]['total_data_bytes'])
        data_json3[i]['avg_data_rate'] = int(data_json3[i]['avg_data_rate'])
        data_json3[i]['tx_avg_data_rate'] = int(data_json3[i]['tx_avg_data_rate'])
        data_json3[i]['rx_avg_data_rate'] = int(data_json3[i]['rx_avg_data_rate'])
        data_json3[i]['tx_frames_transmitted'] = int(data_json3[i]['tx_frames_transmitted'])
        data_json3[i]['tx_data_transmitted'] = int(data_json3[i]['tx_data_transmitted'])
        data_json3[i]['tx_data_bytes_transmitted'] = int(data_json3[i]['tx_data_bytes_transmitted'])
        data_json3[i]['tx_abs_data_bytes'] = int(data_json3[i]['tx_abs_data_bytes'])
        data_json3[i]['tx_mgmt'] = int(data_json3[i]['tx_mgmt'])
        data_json3[i]['rx_frames'] = int(data_json3[i]['rx_frames'])
        data_json3[i]['rx_bytes'] = int(data_json3[i]['rx_bytes'])
        data_json3[i]['rx_data'] = int(data_json3[i]['rx_data'])
        data_json3[i]['rx_data_bytes'] = int(data_json3[i]['rx_data_bytes'])
        data_json3[i]['rx_abs_data_bytes'] = int(data_json3[i]['rx_abs_data_bytes'])
        data_json3[i]['total_data_throughput'] = int(data_json3[i]['total_data_throughput'])
        data_json3[i]['tx_data_throughput'] = int(data_json3[i]['tx_data_throughput'])
        data_json3[i]['rx_data_throughput'] = int(data_json3[i]['rx_data_throughput'])
        data_json3[i]['client_auth_type'] = int(data_json3[i]['client_auth_type'])
        data_json3[i]['client_auth_subtype'] = int(data_json3[i]['client_auth_subtype'])
        data_json3[i]['client_encrypt_type'] = int(data_json3[i]['client_encrypt_type'])
        data_json3[i]['client_fwd_mode'] = int(data_json3[i]['client_fwd_mode'])
        # Append int values for ratio units
        data_json3[i]['tx_time_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['tx_time'])[0])
        data_json3[i]['rx_time_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['rx_time'])[0])
        data_json3[i]['channel_free_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['channel_free'])[0])
        data_json3[i]['channel_interference_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['channel_interference'])[0])
        data_json3[i]['channel_busy_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['channel_busy'])[0])
        data_json3[i]['tx_frames_dropped_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['tx_frames_dropped'])[0])
        # data_json[i]['tx_bytes_transmitted_int'] = int(re.findall("([0-9]+)\/", data_json[i]['tx_bytes_transmitted'])[0])
        data_json3[i]['tx_bytes_dropped_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['tx_bytes_dropped'])[0])
        data_json3[i]['tx_time_transmitted_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['tx_time_transmitted'])[0])
        data_json3[i]['tx_time_dropped_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['tx_time_dropped'])[0])
        data_json3[i]['tx_data_dropped_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['tx_data_dropped'])[0])
        data_json3[i]['tx_data_retried_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['tx_data_retried'])[0])
        data_json3[i]['tx_data_transmitted_retried_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['tx_data_transmitted_retried'])[0])
        data_json3[i]['tx_data_dropped_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['tx_data_dropped'])[0])
        data_json3[i]['tx_data_retried_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['tx_data_retried'])[0])
        data_json3[i]['tx_data_transmitted_retried_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['tx_data_transmitted_retried'])[0])
        data_json3[i]['tx_time_data_transmitted_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['tx_time_data_transmitted'])[0])
        data_json3[i]['tx_time_data_dropped_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['tx_time_data_dropped'])[0])
        data_json3[i]['rx_data_retried_int'] = int(re.findall("([0-9]+)\/", data_json3[i]['rx_data_retried'])[0])
        # Append ISODate
    except Exception:
        pass
    data_json3[i]['ts'] = ts
...
# for i in range(len(data_json3)):
#     if (data_json3[i]['radio_band']== 1):              
#         data_json3[i]['channel_Hz'] ==5
#     else:
#         data_json3[i]['channel_Hz'] ==2.4
